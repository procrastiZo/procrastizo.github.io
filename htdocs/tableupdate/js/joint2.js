...
jQuery(function ($){
	var $modal = $('#editor-modal'),
		...
		ft = FooTable.init('#showcase-example-1', {
			columns: $.ajax("https://api.myjson.com/bins/19icne", {dataType:'json', timeout: 3000}), //no cache for collumn because it shouldn't change
			rows: $.ajax("https://api.myjson.com/bins/1g24ay", {cache: false, dataType:'json', timeout: 3000}),//this cache line is used so that the website wont keep the rows so it stays updated
			editing: {
				enabled: true,
				addRow: function(){
